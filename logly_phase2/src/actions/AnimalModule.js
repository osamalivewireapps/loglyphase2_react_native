...
export const getAnimalsById = (id) => async (dispatch) => {


    let config = { headers: { 'auth': await DataHandler.getAuth() } };

    console.log("getAnimals config---------------->", id)

    return new Promise((resolve) => {
        dispatch(EnableLoader());
        axios
            .get(`${baseUrl}/animal/v2/${id}`, config)
            .then(response => {
...
export const getVaccinationHistory = (id) => async (dispatch) => {
    ...
    return new Promise((resolve) => {
        dispatch(EnableLoader());
        axios
            .get(`${baseUrl}/activity/getActivityByCategory?animalId=${id}`, config)
            .then(response => {
